...
export default function EditDateTimeModalForAdmin({
  modal,
  toggle,
  data,
  callbackFn,
...
using the `setValue` function from `react-hook-form`. */
  ...
  useEffect(() => {
    ...
    if (arrivalTimeValue) {
      startHour = dayjs(arrivalTimeValue)
        .tz(data.timeZone ?? DEFAULT_TIMEZONE)
        .get("hour");
      startMinute = dayjs(arrivalTimeValue)
        .tz(data.timeZone ?? DEFAULT_TIMEZONE)
        .get("minute");
    }
    if (endTimeValue) {
    ...
    data.jobSlots = data.jobSlots.map((slot) => {
      return {
        ...
        startDate: dayjs(slot.startDate?.getTime())
          .tz(data.timeZone ?? DEFAULT_TIMEZONE, true)
          .utc()
          .valueOf(),
