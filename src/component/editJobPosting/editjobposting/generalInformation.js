...
import EditClinicianTypeModal from "component/modals/EditClinicianTypeModal";
import EditDateTimeModal from "component/modals/EditDateTimeModal";
import EditFacilityTypeModal from "component/modals/EditFacilityTypeModal";
import EditLocationModal from "component/modals/EditLocationModal";
import EditMinimumExperienceModal from "component/modals/EditMinimumExperienceModal";
import EditPayModal from "component/modals/EditPayModal";
import {
  ...
  return (
    <div className="general-content">
      <div className="general-detail">
        <ul>
                ...
                {!CONFIRMED?.includes(data?.status) ? (
                  <>
                    ...
                    {[
                      ...
                      data?.jobAddress2,
                      data?.city,
                      state.find((item) => item.value === data.state)
                        ?.stateCode,
                      data.zipCode,
                    ]
                      .filter(Boolean)
          ...
          <li className="flex-wrap">
            <span>Hourly Rate:</span> ${data?.effectiveHourlyRate}
            {editable && (
              <span
                className="pencilicon"
      ...
      <EditDateTimeModal
        ...
        data={data}
        step={data.applicantCount >= 1 ? 2 : null} // If there are applicants then facility can only change the time, skip the date selection part
        callbackFn={() => {
          setEditDMModal(!editDMModal);
          getData();
        }}
      />
      <EditFacilityTypeModal
