...
export default function GeneralInformation({ currentActiveTab, location }) {
  const {
    register,
    trigger,
    control,
  ...
  const handleSave = async (keys, loaderValue) => {
    ...
    if (result) {
      ...
      if (loaderValue === 8) {
        values.facilityRepresentativeName = values?.facilityRepresentativeName;
        values.facilityRepresentativeEmail = values?.facilityRepresentativeEmail;
      }
  ...
  const removeOther = (arr) => {
    return arr.filter((val) => val !== "Other");
  };
  const onSave = (reqData, loaderValue) => {
    let tempInitData = {
      ...initData,
      dressCode: removeOther(user.facilityId.dressCode),
      // emrList: removeOther(user.facilityId.emrList),
      facilityType: removeOther(user.facilityId.facilityType),
      patientsType: removeOther(user.facilityId.patientsType),
    };
    ...
    let data = {
      ...
      ...reqData,
      userId: initData.id,
      id: initData.facilityId.id,
      messageNumber: tempValues.messageNumber,
    };
                      ...
                      <Col md={12}>
                        <div className="clinic-question-ans">
                          <label>
                            ...
                            (Highlight for clinicians why they would want to
                            pick up a shift at your clinic)
                          </label>
